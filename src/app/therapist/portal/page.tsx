...
function AppointmentCard({ appt, actions }: {
  appt: Appointment;
...
function AppointmentsPanel() {
  ...
  // Unassigned appointments waiting for a therapist
  const { data: unassigned, isLoading: loadingUnassigned } = useQuery<Appointment[]>({
    queryKey: ["therapist", "unassigned"],
    queryFn: () => therapistApi().get("/therapist/portal/appointments/unassigned").then(r => r.data),
  ...
  const pendingUnassigned = unassigned ?? [];

  return (
    <div className="space-y-4">
      {/* Unassigned — waiting for a therapist to claim */}
      {(pendingUnassigned.length > 0 || isLoading) && (
        <div className="bg-amber-50 rounded-2xl border border-amber-200 p-5 space-y-3">
          <div className="flex items-center gap-2">
            <CalendarDays size={15} className="text-amber-600" />
            <h2 className="text-sm font-semibold text-amber-800">Turnos sin asignar</h2>
            <span className="ml-auto text-xs text-amber-500">
              {isLoading ? "…" : `${pendingUnassigned.length} pendientes`}
            </span>
          </div>
      ...
      <div className="bg-white rounded-2xl border border-gray-200 p-5 space-y-3">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-sm font-semibold text-gray-800">Mis turnos</h2>
          <span className="ml-auto text-xs text-gray-400">
            {loadingMine ? "…" : `${activeAppts.length} activos`}
          </span>
        </div>
        ...
        {loadingMine ? (
          ...
          <div className="text-center py-4">
            ...
            <p className="text-xs text-gray-400">No tenés turnos asignados</p>
            <p className="text-[10px] text-gray-300 mt-0.5">Tomá turnos de la sección de arriba cuando haya disponibles</p>
          </div>
        ) : (
          <div className="space-y-2">
